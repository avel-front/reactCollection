...
**`Action'ы`** - это простые объекты, которые имеют специальное поле (type, payload, error, meta). Они являются единственным источником информации для store, поскольку передаются в него через dispatch. Они могут описать только то, что произошло, но не могут описать, как именно изменяется state приложения. 
