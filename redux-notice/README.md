...
Это React-компоненты, которые используют subscribe() для чтения состояний из Redux-дерева и передают props представлению, то есть они получают данные с бэкенда и затем рендерят дочерний компонент. Его рекомендуется генерировать через `connect()`, потому что этот способ оптимизирован и избавляет от ненужных переrender'ов и своей реализации shouldComponentUpdate().  
