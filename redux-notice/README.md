...
Технически это React-компонент, который использует store.subscribe() для чтения части состояний Redux-дерева и поставляет props представлению, которое он render'ит.
...
Если контейнер переполнен, рекомендуется его заздробить на части. Компонент может быть **смешанным**, когда он мал. Например, input с кнопкой "добавить".  Sa
